...
# Stone classifier model wrapper
class BaseStoneClassifier:
    ...
    """
    def __init__(self, model_dir, img_dir, img_size = (IMG_WIDTH, IMG_HEIGHT)):
        """Constructor.
        Parameters:
        ...
        """
        self.model = None
        self.model_dir, self.img_dir, self.img_size = model_dir, img_dir, img_size
    ...
    def display_predictions(self):
        ...
        for _, elements in self.predict_dataset.enumerate():
            ...
            for image, true_label in zip(elements[0], elements[1]):
                try:
                ...
                plt.title('{} ({})'.format(
                    ...
                    self.class_names[int(true_label)]))
...
def main():
    ...
    model_dir =  str(Path(__file__).absolute().parent.joinpath('stone_classifier'))

    classifier = BaseStoneClassifier(model_dir, img_dir)
    if classifier.exists():
        classifier.load()
    else:
        classifier.train(display_history = True)
        classifier.save()
